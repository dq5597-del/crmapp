...
exports.ALL_COLUMNS = exports.SUB_COLUMNS = exports.PRODUCT_COLUMNS = void 0;
exports.normalizeHeader = normalizeHeader;
exports.columnForHeader = columnForHeader;
...
exports.ALL_COLUMNS = [...exports.PRODUCT_COLUMNS, ...exports.SUB_COLUMNS];
/** 表頭 → key（容錯：去空白、全形括號） */
const HEADER_MAP = (() => {
    const m = {};
    for (const c of exports.ALL_COLUMNS)
        m[normalizeHeader(c.header)] = c;
    // 常見別名
    const alias = {
    ...
    for (const [a, target] of Object.entries(alias)) {
        const col = exports.ALL_COLUMNS.find(c => c.header === target);
        if (col)
            m[normalizeHeader(a)] = col;
    }
    return m;
})();
...
function toBool(v, dflt = false) {
    ...
    if (['是', 'y', 'yes', 'true', '1', 'v', '✓', 'on'].includes(s))
        return true;
    if (['否', 'n', 'no', 'false', '0', ''].includes(s))
        return false;
    return dflt;
}
function toList(v) {
    ...
    for (const [header, value] of Object.entries(raw)) {
        ...
        switch (col.key) {
            case 'features': {
                ...
                if (list.some(f => f.length > 5))
                    out.warnings.push('產品特色超過 5 字的項目會被截斷');
                if (list.length > 10)
                    out.warnings.push('產品特色超過 10 項，只取前 10 項');
            ...
            case 'filter_specs': {
                ...
                for (const spec of specs) {
                    ...
                    if (!group || !values.length)
                        out.errors.push(`篩選規格缺少名稱或值：${spec}`);
                    else
                        out.filter_specs.push({ group, values });
                }
                break;
            }
            default: {
                if (col.type === 'number') {
                    ...
                    if (s)
                        out.product[col.key] = s;
                }
            }
        }
    }
    // 必填 / 一致性檢查
    if (!out.product.product_name)
        out.errors.push('產品名稱為必填');
    if (out.product.main_category && !out.product.sub_category)
        out.warnings.push('只有主分類、沒有次分類，將不設定分類');
    if (!out.product.model && !out.product.web_sku)
        out.warnings.push('沒有型號也沒有官網SKU，無法比對既有產品，只能新增');
    if (out.product.variant_group_code && !out.product.variant_value)
        out.errors.push('有系列代碼時，「變體選項」為必填');
    if (out.product.variant_value && !out.product.variant_group_code)
        out.errors.push('有變體選項時，「系列代碼」為必填');
    if (out.product.variant_group_code && !out.product.variant_attribute_name)
        out.product.variant_attribute_name = '顏色';
    for (const u of [out.main_image_url, ...out.image_urls]) {
        if (u && !/^https?:\/\//i.test(u))
            out.errors.push(`圖片網址須為 http(s) 開頭：${u}`);
    }
    return out;
}
/** 比對鍵 */
