...
const ALLOWED_MEDIA_TYPES = new Set([...ALLOWED_IMAGE_TYPES, 'application/pdf'])

const WORDPRESS_IMAGE_SIZE = 600
const WORDPRESS_WEBP_QUALITY = 84
export type WordPressImagePreset = 'product' | 'content'

const DOCUMENT_EXTENSIONS = new Set(['.pdf', '.zip', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx'])
...
export function isAllowedWordPressMediaType(mimeType: string) {
  return ALLOWED_MEDIA_TYPES.has(mimeType.toLowerCase())
}

export function isAllowedWordPressDocument(fileName: string, mimeType: string) {
  const ext = fileName.slice(fileName.lastIndexOf('.')).toLowerCase()
  return DOCUMENT_EXTENSIONS.has(ext) && !mimeType.toLowerCase().startsWith('image/')
}
...
export async function prepareWordPressImage(data: Buffer, preset: WordPressImagePreset = 'product') {
  const size = preset === 'content' ? 1200 : WORDPRESS_IMAGE_SIZE
...
export async function uploadWordPressMedia(options: {
  ...
  if (!isAllowedWordPressMediaType(options.mimeType)) {
    throw new Error('僅支援 JPG、PNG、WebP、GIF 或 PDF')
  }

  const isImage = isAllowedWordPressImageType(options.mimeType)
  const uploadData = isImage ? await prepareWordPressImage(options.data, options.preset) : options.data
  const uploadMimeType = isImage ? 'image/webp' : options.mimeType
  const uploadFileName = isImage
...
export async function uploadWordPressDocument(options: {
  ...
  if (!isAllowedWordPressDocument(options.fileName, options.mimeType)) throw new Error('不支援的文件格式')
  const { store, username, applicationPassword } = wordpressMediaConfig()
  if (!store || !username || !applicationPassword) throw new Error('尚未設定 WordPress 媒體上傳帳號')
  const auth = Buffer.from(`${username}:${applicationPassword}`).toString('base64')
  const filename = safeWordPressFileName(options.fileName)
  const upload = await fetch(`${store}/wp-json/wp/v2/media`, {
    method: 'POST',
    headers: { Authorization: `Basic ${auth}`, 'Content-Type': options.mimeType || 'application/octet-stream', 'Content-Disposition': `attachment; filename="${filename}"` },
    body: Uint8Array.from(options.data),
    cache: 'no-store',
  })
  const media = await upload.json().catch(() => null)
  if (!upload.ok) throw new Error(media?.message ?? `WordPress 文件上傳失敗（HTTP ${upload.status}）`)
  if (options.title || options.productId) {
    await fetch(`${store}/wp-json/wp/v2/media/${media.id}`, {
      method: 'POST', headers: { Authorization: `Basic ${auth}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: options.title || undefined, parent: options.productId || undefined }), cache: 'no-store',
    })
  }
  return { id: media.id as number, url: media.source_url as string, file_name: filename }
}
