...
export const PRODUCT_COLUMNS: ImportColumn[] = [
  ...
  { header: '主分類', key: 'main_category', type: 'text', width: 14, note: '使用光輝系統既有進銷存分類' },
  ...
  { header: '官網分類', key: 'web_category', type: 'text', width: 22, note: '單一分類；有主／次分類時以產品分類為準' },
...
export const SUB_COLUMNS: ImportColumn[] = [
  ...
  { header: '主圖網址', key: 'main_image_url', type: 'text', width: 40, note: 'http(s) 圖片網址；自動調整為 600 × 600 px、轉成 WebP 並存入 WordPress 媒體庫' },
  { header: '其他圖片網址', key: 'image_urls', type: 'list', width: 40, note: '多張用 | 分隔' },
  { header: '篩選規格', key: 'filter_specs', type: 'list', width: 36, note: '格式：規格群組=值，多組用 | 分隔' },
]
...
export interface ParsedRow {
  ...
  image_urls: string[]
  filter_specs: Array<{ group: string; values: string[] }>
  errors: string[]
  warnings: string[]
}
  ...
  for (const [header, value] of Object.entries(raw)) {
    ...
    switch (col.key) {
      ...
      case 'image_urls':
        ...
        break
      case 'filter_specs':
        out.filter_specs = toList(value).flatMap(item => {
          const separator = item.indexOf('=')
          if (separator <= 0 || separator === item.length - 1) {
            out.errors.push(`篩選規格格式錯誤：${item}`)
            return []
          }
          const values = item.slice(separator + 1).split(/[,，]/).map(value => value.trim()).filter(Boolean)
          return [{ group: item.slice(0, separator).trim(), values }]
        })
        break
      default: {
        if (col.type === 'number') {
