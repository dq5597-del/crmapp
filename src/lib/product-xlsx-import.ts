import { columnForHeader, parseRow, type ParsedRow } from '@/lib/product-import'
...
export interface ProductXlsxParseResult {
  rows: ParsedRow[]
...
function imageBytes(image: { buffer?: Uint8Array; base64?: string }) {
...
export async function parseProductXlsx(file: File): Promise<ProductXlsxParseResult> {
  const ExcelJS = (await import('exceljs')).default
  const workbook = new ExcelJS.Workbook()
  await workbook.xlsx.load(await file.arrayBuffer() as any)

  const worksheet = workbook.worksheets[0]
  if (!worksheet || worksheet.rowCount < 2) throw new Error('檔案沒有資料列')

  const headers: string[] = []
  worksheet.getRow(1).eachCell({ includeEmpty: true }, (cell, column) => {
    headers[column] = cellText(cell.value)
  })
  const unknownHeaders = headers.filter(Boolean).filter(header => !columnForHeader(header))
  ...
  for (const row of rows) {
    ...
    if (!model) continue
    if (seenModels.has(model)) row.errors.push(`型號「${row.product.model}」與第 ${seenModels.get(model)} 列重複`)
    else seenModels.set(model, row.rowNo)
  }

  const warnings: string[] = []
  const rowsByNumber = new Map(rows.map(row => [row.rowNo, row]))
  const counters = new Map<string, number>()
  const embeddedPhotos: File[] = []
  const images = worksheet.getImages().slice().sort((left, right) => (
    left.range.tl.nativeRow - right.range.tl.nativeRow
  ...
  for (const imageRef of images) {
    ...
    const column = columnForHeader(headers[columnNo] ?? '')
    if (!column || !EMBEDDED_IMAGE_KEYS.has(column.key)) continue
    ...
    const bytes = imageBytes(image as any)
    if (!bytes?.length) {
    ...
    embeddedPhotos.push(new File(
      [toArrayBuffer(bytes)],
      `${identifier}${marker}.${extension}`,
      { type: mimeType, lastModified: Date.now() + embeddedPhotos.length },
    ))
  }
