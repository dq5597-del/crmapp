...
import { fetchDocData, buildDocx, type DocType } from '@/lib/doc-export'

export const runtime = 'nodejs'

// GET /api/docs/[type]/[id]/export-docx — 通用 Word 匯出（訂購/退貨/出貨/詢價/銷貨）
export async function GET(_req: NextRequest, { params }: { params: { type: string; id: string } }) {
  ...
  return new NextResponse(buf as any, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${encodeURIComponent(`${data.title}_${data.docNo}.docx`)}"`,
    },
  })
}
