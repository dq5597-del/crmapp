...
export default function PurchaseOrderDetailPage() {
  ...
  const [form, setForm] = useState({ vendor_name: '', vendor_contact: '', vendor_phone: '', notes: '', status: '草稿', salesperson_id: '' })
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (oRes.data) {
        setOrder(oRes.data)
        setForm({ vendor_name: oRes.data.vendor_name ?? '', vendor_contact: oRes.data.vendor_contact ?? '', vendor_phone: oRes.data.vendor_phone ?? '', notes: oRes.data.notes ?? '', status: oRes.data.status ?? '草稿', salesperson_id: oRes.data.salesperson_id ?? '' })
      }
      setItems(iRes.data ?? [])
