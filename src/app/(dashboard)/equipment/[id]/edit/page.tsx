...
export default function EditEquipmentPage() {
  ...
  useEffect(() => {
    if (!id) return
    supabase.from('equipment').select('*').eq('id', id).maybeSingle().then(({ data, error }) => {
    ...
    // 這台設備目前連結的點工（可能不只一個）
    supabase.from('equipment_work_logs').select('work_log_id').eq('equipment_id', id).then(({ data }) => {
      setWorkLogIds((data ?? []).map((r: any) => r.work_log_id))
    })
  }, [id])
  ...
  function toggleWorkLog(wid: string) {
    setWorkLogIds(prev => prev.includes(wid) ? prev.filter(x => x !== wid) : [...prev, wid])
  }
  ...
  async function handleDelete() {
    if (!confirm('確定要刪除這台設備嗎？這個動作無法復原。')) return
    setDeleting(true)
  ...
  if (notFound) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        ...
        <button onClick={() => router.push('/equipment')} className="mt-4 text-sm text-blue-600 hover:underline">
          回設備清單
        </button>
      </div>
    )
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">編輯設備</h1>
            <p className="text-xs text-gray-500">修改這台設備的資訊</p>
          </div>
        </div>
        <button
      ...
      <div className="flex justify-end gap-3 pb-8">
        <button onClick={() => router.push('/equipment')} className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">
          取消
        </button>
        <button
          onClick={handleSave}
