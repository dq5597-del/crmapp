...
function buildOrderFileName(order: { order_no?: string | null; project_name?: string | null }, fallbackName?: string | null): string {
  ...
  const namePart = (order.project_name?.trim() || fallbackName?.trim() || '').replace(/[\\/:*?"<>|]/g, '').trim()
  return ['(光輝)銷貨單', namePart, datePart, seqPart].filter(Boolean).join('_')
}
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const { data: order } = await supabase
    .from('sales_orders')
    .select('order_no, project_name, clients(company_name)')
    .eq('id', params.id)
    .single()

  if (!order) return {}
  const fileName = buildOrderFileName(order, (order as any).clients?.company_name)
...
export default async function SalesOrderPrintPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: order }, { data: items }] = await Promise.all([
    supabase.from('sales_orders').select('*, clients(company_name, phone, address), salesperson:user_profiles(full_name)').eq('id', params.id).single(),
  ...
  const deliveryAddress = (order as any).delivery_address || (order as any).clients?.address || ''
