...
export async function generateMetadata(props: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!order) return {}
  const fileName = buildDocFileName('銷貨單', order.order_no, (order as any).clients?.company_name, order.project_name)
...
export default async function SalesOrderPrintPage(props: { params: Promise<{ id: string }> }) {
  const params = await props.params;
  const supabase = createClient()
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* 隱藏金額（給廠商看的版本）：tfoot 本身保留，只隱藏其中的列，
           否則分頁程式會量到錯誤座標 */
