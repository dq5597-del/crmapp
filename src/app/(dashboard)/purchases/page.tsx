...
import { chooseTemporaryA4Printer, queueHtmlDocument } from '@/lib/cloud-document-print'
...
export default function PurchasesPage() {
  ...
  async function printPurchase(row: any, printerId?: string) {
    setBarcodeLoading(row.id)
    try {
      ...
      const printer = await queueHtmlDocument('purchase', html, row.id, row.purchase_no, printerId)
      alert(`已直接送到「${printer.name}」列印。`)
    } catch (e) { alert('進貨單列印失敗：' + ((e as Error).message || '未知錯誤')) }
    finally { setBarcodeLoading(null) }
  }

  async function temporaryPrintPurchase(row: any) {
    try {
      const selected = await chooseTemporaryA4Printer()
      if (selected) await printPurchase(row, selected.id)
              ...
              {loading ? (
                ...
                <tr key={r.id} className="border-b border-gray-50 hover:bg-teal-50/40">
                  ...
                  <td className="px-4 py-3 text-center">
                    ...
                    <button type="button" onClick={() => temporaryPrintPurchase(r)} disabled={barcodeLoading === r.id}
                      title="本次臨時改用其他 A4 印表機"
                      className="mr-1 inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 disabled:opacity-50">
                      臨時換印表機
                    </button>
