...
export default function PayablePrintPage() {
    ...
    (async () => {
      const [rRes, sRes] = await Promise.all([
        supabase.from('payables')
          .select('*, vendors(company_name, phone, address, tax_id), purchase_orders(order_no)')
          .eq('id', id).maybeSingle(),
        supabase.from('system_settings').select('*').limit(1).maybeSingle(),
      ])
      setRow(rRes.data); setCompany(sRes.data)
      const { data: pay } = await supabase.from('payable_payments').select('*').eq('payable_id', id).order('payment_date')
      setPayments(pay ?? [])
      setLoading(false)
    })()
  }, [id])
  ...
  if (!row) return <div className="p-10 text-center text-gray-400">找不到這筆應付帳款</div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6">
      <div className="max-w-[820px] mx-auto px-4 mb-4 print:hidden">
        <PrintDocButtons fileName={`應付對帳單_${c.company_name ?? ''}_${row.payable_no ?? ''}`} />
      </div>
      ...
      <div id="print-page-content" className="max-w-[820px] mx-auto bg-white p-10 shadow print:shadow-none">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold mt-3 tracking-widest">應 付 對 帳 單</h1>
        </div>

        <table className="w-full text-sm mb-5 border border-gray-300">
          <tbody>
            <tr>
              <Th>對帳單號</Th><Td>{row.payable_no ?? '—'}</Td>
              <Th>廠商</Th><Td>{c.company_name ?? '—'}</Td>
            </tr>
            <tr>
            ...
            <tr>
              <Th>關聯訂購單</Th><Td>{row.purchase_orders?.order_no ?? '—'}</Td>
        ...
        <table className="w-full text-sm border border-gray-300 mb-5">
          <tbody>
            <tr className="border-b border-gray-200">
              <td className="px-4 py-3 bg-gray-50 text-gray-600 w-1/3">應付金額（含稅）</td>
              <td className="px-4 py-3 text-right font-semibold">{money(row.amount)}</td>
            </tr>
            <tr className="border-b border-gray-200">
              <td className="px-4 py-3 bg-gray-50 text-gray-600">已付金額</td>
              <td className="px-4 py-3 text-right text-green-700">{money(row.paid_amount)}</td>
            </tr>
            <tr>
              <td className="px-4 py-3 bg-gray-50 font-bold">未付餘額</td>
        ...
        {payments.length > 0 && (
          <>
            <div className="text-sm font-semibold mb-2">付款紀錄</div>
            <table className="w-full text-sm border border-gray-300 mb-6">
              <thead>
                <tr className="bg-gray-100">
                  <th className="px-3 py-2 text-left font-medium text-gray-600 border-b border-gray-300">付款日</th>
        ...
        <div className="grid grid-cols-2 gap-10 mt-12 text-sm">
          <div className="border-t border-gray-400 pt-2 text-center text-gray-500">本公司用印</div>
          <div className="border-t border-gray-400 pt-2 text-center text-gray-500">廠商對帳簽收</div>
        </div>
      </div>
    </div>
  )
